...
- Name: migration-diff.spec.ts >> Angular baseline behavior checks >> header contains About navigation link
- Location: tests/migration-diff.spec.ts:28:7
...
Locator: getByRole('link', { name: /about/i }).first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for getByRole('link', { name: /about/i }).first()
